...
static double	dist_visible(t_scene s, t_ray ray,
	t_vec3 point, t_vec3 surfaceToLight)
{
	...
	{
		if (&s.objects_array[i] == ray.closestitem)
			continue ;
		dist = NO_HIT;
		if (s.objects_array[i].type == SPHERE)
			dist = hit_sp(point, surfaceToLight, &s.objects_array[i].shape.sp);
		else if (s.objects_array[i].type == PLANE)
			dist = hit_pl(point, surfaceToLight, s.objects_array[i].shape.pl);
		else if (s.objects_array[i].type == CYLINDER)
			dist = hit_cy(point, surfaceToLight, &s.objects_array[i].shape.cy);
...
double	get_light_angle(t_scene s, t_ray ray, t_vec3 light)
{
	...
	if (ray.closestitem->type == PLANE
		&& !check_same_side(ray.closestitem->shape.pl, s.cam.pos, light))
		return (-1.0);
	...
	dist_to_visible = dist_visible(s, ray, point, surface_to_light);
