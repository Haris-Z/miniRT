...
static void	map_tex(t_vec3 cam, t_tex tex, t_ray *ray)
{
	...
	// double	hor;
	// double	ver;
	// hor = atan2(surface_normal.x, surface_normal.y);
	// ver = acos(surface_normal.z);
	w = 1 - ((atan2(surface_normal.x, surface_normal.y) / (2.0 * PI)) + 0.5);
	...
	ray->closestitem->color = color_rgb(r,g,b);
}
