...
/*   Updated: 2026/03/16 21:33:29 by hazunic          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	const char *parse_error(int err)
{
	if (err == RT_ERR_MALLOC)
		return ("Out of Memory");
	if (err == RT_ERR_TOK)
...
{
	if (err >= RT_ERR_MALLOC && err <= RT_ERR_FORMAT)
...
void	print_parse_err(int lineno, char *id, int err)
{
	ft_putstr_fd(C_BOLD C_RED "Error\n" C_RESET, STDERR_FILENO);
	ft_putstr_fd(C_RESET, STDERR_FILENO);
	...
	if (id)
	{
		...
		ft_putstr_fd("): ", STDERR_FILENO);
		ft_putstr_fd(C_DIM, STDERR_FILENO);
		ft_putstr_fd((char *)rt_parse_strerror(err), STDERR_FILENO);
		if (err == RT_ERR_LIGHT_COUNT)
		{
