...
void	addDirVectorRow(t_cam_rt *cam)
{
	...
	while(++i < cam->pixels[0])
	{
		...
		cam->rays[i].direction.z  = sqrt(cam->orientation.x * cam->orientation.x + cam->orientation.y * cam->orientation.y) * tan(cam->verOffset);
		cam->rays[i].dist = -1.0;
...
int	dirVector_init(t_cam_rt *cam)
{
	...
	cam->verOffset = atan(
		tan(cam->fov / (2.0 * RADIAN)) /
		(((double)cam->pixels[0] / (double)cam->pixels[1])));// * cam->focalLength);
	cam->deltaVerAngle = (2 * cam->verOffset / cam->pixels[1]);
	cam->verOffset += atan2(
		cam->orientation.z,
		sqrt((cam->orientation.x * cam->orientation.x)
